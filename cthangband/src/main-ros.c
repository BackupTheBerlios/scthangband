 ...
 */
#define RISCOS_VARIANT	GAME_NAME

/*
 * AUTHORS
 *  For the info box. [eg. "Ben Harrison"]
 */
#define AUTHORS		MAINT_NAME
...
static void init_menus(void)
{
	...
	o += sprintf(o, "%s,%s,%s|", windows[1].name, windows[2].name,
				 windows[3].name);
	sprintf(o, "%s,%s,%s,%s", windows[4].name, windows[5].name,
			windows[6].name, windows[7].name);
#else
...
static void initialise_terms(void)
{
	...
	for (i = 1; i < MAX_TERM_DATA; i++)
	{
		...
		data[i].unopened = 1;
#ifndef OLD_TERM_MENU
		sprintf(t, "%s (%s %s)", windows[i].name, VARIANT, VERSION);
#else
