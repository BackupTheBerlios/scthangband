...
/* Temp return value for create_pixel_gtk */
static GdkColor temp_color;

/*
 * Hack -- Convert an RGB value to an X11 Pixel, or die.
 */
static GdkColor *create_pixel_gtk(byte red, byte green, byte blue)
{
#ifdef SUPPORT_GAMMA
...
static void store_pixel_colors(void)
{
	...
	for (i = 0; i < 256; i++)
	{
		...
		clr->pixel = *create_pixel_gtk(clr->red, clr->green, clr->blue);
	}
}
 ...
 */
static void ReadBMP_gtk(char *Name)
{
	FILE *f;
	...
	for (i = 0; i < ncol; i++)
	{
		...
		/* Analyze the color */
		clr_pixels[i] = create_pixel_gtk(clrg.r, clrg.g, clrg.b)->pixel;
	}
		...
		for (x = 0; x < infoheader.biWidth; x++)
		{
			...
			if (infoheader.biBitCount == 24)
			{
				...
				gdk_image_put_pixel(tiles_norm, x, y2,
					create_pixel_gtk(ch, c2, c3)->pixel);
			}
...
static void font_ok_callback(GtkWidget UNUSED *widget, GtkWidget *font_selector)
{
	gchar *fontname;
...
static void change_font_event_handler(GtkWidget UNUSED *widget, gpointer user_data)
{
 ...
 */
static void show_event_handler(GtkWidget UNUSED *widget, gpointer user_data)
{
	term_data *td = (term_data *)user_data;
 ...
 */
static void hide_event_handler(GtkWidget UNUSED *widget, gpointer user_data)
{
	term_data *td = (term_data *)user_data;
...
static void file_ok_callback(GtkWidget UNUSED *widget, GtkWidget *file_selector)
{
...
static void open_event_handler(GtkButton UNUSED *was_clicked, gpointer UNUSED user_data)
{
 ...
 */
static bool pick_graphics(int graphics, int *xsize, int *ysize, char *filename)
{
 ...
 */
static void change_graf_mode_event_handler(GtkButton UNUSED *was_clicked,
										gpointer user_data)
{
 ...
 */
static void change_trans_mode_event_handler(GtkButton UNUSED *was_clicked,
...
static gboolean keypress_event_handler(GtkWidget UNUSED *widget, GdkEventKey *event,
										 gpointer UNUSED user_data)
{
	int i, mc, ms, mo, mx;
...
static void term_menu_update_handler(GtkWidget *widget, gpointer user_data)
{
	...
	{
		/* Build the path name */
		strnfmt(buf, 64, "<Angband>/Terms/%s", windows[i].name);
...
static void font_menu_update_handler(GtkWidget *widget, gpointer user_data)
{
	...
	{
		/* Build the path name */
		strnfmt(buf, 64, "<Angband>/Options/Font/%s", windows[i].name);
...
#ifdef SET_NEW_GAME
static void new_event_handler(GtkButton UNUSED *was_clicked, gpointer UNUSED user_data)
{
	if (!game_in_progress)
	{
...
static gboolean delete_event_handler(GtkWidget UNUSED *widget, GdkEvent UNUSED *event,
										gpointer UNUSED user_data)
{
	save_game_gtk();
 ...
 */
static void quit_event_handler(GtkButton UNUSED *was_clicked, gpointer UNUSED user_data)
{
	save_game_gtk();
...
static void destroy_event_handler(GtkButton UNUSED *was_clicked, gpointer UNUSED user_data)
{
	cleanup_angband();
...
static void setup_menu_paths(void)
{
	...
	for (i = 0; i < MAX_TERM_DATA; i++)
	{
		...
		strnfmt(buf, 64, "/Terms/%s", windows[i].name);

		/* XXX XXX Store it in the menu definition */
		term_entry[i].path = (char *) string_make(buf);

		/* XXX XXX Build the real path name to the entry */
		strnfmt(buf, 64, "/Options/Font/%s", windows[i].name);
 ...
 */
static void hook_quit(cptr UNUSED str)
{
...
static void Term_fresh_gtk(void)
{
	(void)Term_fresh();
}
...
errr init_gtk(int argc, char **argv)
{
	 ...
	 */
	data[0].t.resize_hook = Term_fresh_gtk;
